...
import UnAuthorized from "viewPages/UnAuthorized";
...
function DashboardAdmin(props) {
  ...
  // console.log('Role', ability.can('view', 'HospitalList'))
  ...
  const getActiveRoute = routes => {
    ...
    for (let i = 0; i < routes.length; i++) {
      if (routes[i].collapse) {
        ...
        if (`${routes[i].layout}${routes[i].path}`.match('/:')) {
          let pathName = `${routes[i].layout}${routes[i].path}`.split(":");
          if (window.location.href.includes(pathName[0])) {
      ...
      if (prop.layout === "/admin") {
        return (
          <Route
            ...
            render={(props) => (
              ability.can('view', prop.name) ?
                <prop.component {...prop.component} /> : <UnAuthorized />
            )}
            key={key}
          />
          // <Route
...
connect(mapStateToProps)(DashboardAdmin);

function Dashboard(props) {
  return (
    <React.Fragment>
      {
        !props.isSessionActive ? <AuthLayout /> : <DashboardAdmin {...props} />
      }
    </React.Fragment>
  )
}
